...
const double _kNavBarBottomPadding = 8;
...
/// to also has a [CupertinoNavigationBar] or a [AppCustomCupertinoSliverNavigationBar]
...
///
/// By default, only one [CupertinoNavigationBar] or [AppCustomCupertinoSliverNavigationBar]
...
///  * [AppCustomCupertinoSliverNavigationBar] for a navigation bar to be placed in a
...
class CupertinoNavigationBar extends StatefulWidget
  ...
  /// [AppCustomCupertinoSliverNavigationBar] instances of the same [Navigator]. With the
...
///
/// The [AppCustomCupertinoSliverNavigationBar] must be placed in a sliver group such
...
/// behavior for multiple navigation bars per route.
///
/// [AppCustomCupertinoSliverNavigationBar] has its text scale factor set to 1.0 by default
...
/// [AppCustomCupertinoSliverNavigationBar]'s components inside a [MediaQuery] with the
...
///
/// {@tool dartpad}
/// This example shows [AppCustomCupertinoSliverNavigationBar] in action inside a [CustomScrollView].
///
...
///  * <https://developer.apple.com/design/human-interface-guidelines/ios/bars/navigation-bars/>
class AppCustomCupertinoSliverNavigationBar extends StatefulWidget {
  /// Creates a navigation bar for scrolling lists.
  ///
  /// The [largeTitle] argument is required and must not be null.
  const AppCustomCupertinoSliverNavigationBar({
    super.key,
    this.largeTitle,
    this.leading,
    this.automaticallyImplyLeading = true,
  ...
  @override
  State<AppCustomCupertinoSliverNavigationBar> createState() =>
      _AppCustomCupertinoSliverNavigationBarState();
}

// A state class exists for the nav bar so that the keys of its sub-components
// don't change when rebuilding the nav bar, causing the sub-components to
// lose their own states.
class _AppCustomCupertinoSliverNavigationBarState
    extends State<AppCustomCupertinoSliverNavigationBar> {
  late _NavigationBarStaticComponentsKeys keys;
...
///
/// Magnifies on over-scroll when [AppCustomCupertinoSliverNavigationBar.stretch]
/// parameter is true.
...
class _RenderLargeTitle extends RenderShiftedBox {
  ...
  void performLayout() {
    ...
    final childConstraints = constraints.widthConstraints().loosen();
    child.layout(childConstraints, parentUsesSize: true);

    final maxScale = child.size.width != 0.0
        ? clampDouble(constraints.maxWidth / child.size.width, 1, 1.1)
        : 1.1;
    _scale = clampDouble(
      1.0 +
...
/// [AppCustomCupertinoSliverNavigationBar]'s `leading` slot when
/// `automaticallyImplyLeading` is true.
///
